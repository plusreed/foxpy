...
# A role with the name of Fox Mod and a role with the name of Fox Admin.
...
def mod_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name in ('Fox Mod', 'Fox Admin'), **perms)
...
def admin_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name == 'Fox Admin', **perms)
...
def is_lounge_cpp():
    return is_in_servers(config.LOUNGE_SRV_ID)
