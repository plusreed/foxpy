...
class LastFm:
    ...
    def __init__(self, bot):
        ...
        self.base_user_url = "http://ws.audioscrobbler.com/2.0/?method=user.getinfo&user={0}&api_key={1}"
    ...
    async def profile(self):
        ...
        if config.DEBUG:
            ...
            await self.bot.say("**lfm**: parsing xml, please wait...")
        else:
            lfm_data = []
            u_details = et.parse(p)
            u_playing = et.parse(p2)
            root_details = u_details.getroot()
            root_playing = u_playing.getroot()

            for x in root_details.findall('user') and root_playing.findall('recenttracks'):
                lfm_data.append(x.text)

            if config.DEBUG:
                await self.bot.say("**lfm**: parsed xml, here's what lfm_data looks like:")
                await self.bot.say(lfm_data)
            else:
